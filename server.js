...
app.post('/api/search', async (req, res, next) => 
{
  ...
  try{
    await client.connect();
    const text = "Select r.*, u.firstname, u.lastname from recipes as r left JOIN categories as c ON r.id = c.fkrecipeid left JOIN tags as t ON r.id = t.fkrecipeid left join users as u ON Cast(r.userid as varchar) = Cast(u.id as varchar) Where (lower(r.recipe) like lower($1) OR lower(t.tagname) like lower($1) OR lower(c.categoryname) like lower($1) OR lower(u.firstname) like lower($1) or lower(u.lastname) like lower($1)) and r.privatetable = 'FALSE' GROUP BY r.id, r.recipe, r.text_recipe, u.firstname, u.lastname ORDER BY r.date DESC";
